...
use App\Filament\Resources\EmployeeSelfServiceResource\Pages\EditEmployeeSelfService;
...
class EmployeeSelfServiceResource extends Resource
{
    ...
    {
        return [
            'edit' => EditEmployeeSelfService::route('/{record}/edit'),
            'index' => EditEmployeeSelfService::route("/employee/edit")
        ];
    }
    ...
    public static function profileDisplay(): Grid
    {
        ...
        ->schema([
            ...
            ->schema([
                ...
                ->content(fn (EmployeeSelfService $record): ?string => $record ? $record->user->name : "")
                ->extraAttributes(['class' => 'text-xs']),
                Placeholder::make('Position')->label('')                                
                ->content(fn (EmployeeSelfService $record): ?string => isset($record->position) ? $record->position->job_position : "N/A")
                ->extraAttributes(['class' => 'text-xs']),
    ...
    public static function contactInformation(): Section 
    {
        ...
        ->schema([
            ...
            ->content(fn (EmployeeSelfService $record): ?string => $record ? $record->user->mobile : ""),
            Placeholder::make('email')
            ->content(fn (EmployeeSelfService $record): ?string => $record ? $record->user->email : ""),
            Grid::make([
                'default' => 1
            ])
    ...
    public static function salaryInformation() : Section
    {
        ...
        ->schema([
            ...
            ->schema([
                ...
                ->content(fn (EmployeeSelfService $record): ?string => isset($record->user->fullname) ? $record->user->fullname : ""),
                Placeholder::make('category')->label('Category')
                ->content(fn (EmployeeSelfService $record): ?string => isset($record->position->job_category) ? $record->position->job_category : ""),
                Placeholder::make('job_position')->label('Job Position')
                ->content(fn (EmployeeSelfService $record): ?string => isset($record->position->job_position) ? $record->position->job_position : ""),
                Placeholder::make('payroll_cycle')->label('Payroll Cycle')
                ->content(fn (EmployeeSelfService $record): ?string => isset($record->employment->payroll_cycle) ? $record->employment->payroll_cycle : ""),
                Placeholder::make('payment_structure')->label('Payment Structure')
                ->content(fn (EmployeeSelfService $record): ?string => isset($record->employment->payment_structure) ? $record->employment->payment_structure : ""),
                Placeholder::make('employement_date')->label('Effective Date From')
                ->content(fn (EmployeeSelfService $record): ?string => isset($record->employment->employement_date) ? $record->employment->employement_date : ""),
                Grid::make([
                    'default' => 1
                ])
            ])->columns(6),
