...
class ViewEmployeePayrollToAuditTable extends Page implements HasForms, HasTable, HasActions
{
    ...
    public function getTabs(): array
    {
        return [
            ...
            'Pending' => Tab::make()->modifyQueryUsing(function ($query){
                $query->where('status',config('constants.AUDIT_PENDING'));
            })
            ->badge(function () {
                return Payroll::query()->where('pay_period_id', $this->record->pay_period_id)->where('status',config('constants.AUDIT_PENDING'))
                ->count();
            })
            ->badgeColor('warning'),
            'Approved' => Tab::make()->modifyQueryUsing(function ($query){
                $query->where('status',config('constants.AUDIT_APPROVED'));
            })
            ->badge(function () {
                return Payroll::query()->where('pay_period_id', $this->record->pay_period_id)->where('status',config('constants.AUDIT_APPROVED'))
                ->count();
            })
            ->badgeColor('success'),
            'Denied' => Tab::make()->modifyQueryUsing(function ($query){
                $query->where('status',config('constants.AUDIT_DENIED'));
            })
            ->badge(function () {
                return Payroll::query()->where('pay_period_id', $this->record->pay_period_id)->where('status',config('constants.AUDIT_DENIED'))
                ->count();
            })
            ->badgeColor('danger'),
            'Void' => Tab::make()->modifyQueryUsing(function ($query){
                $query->where('status',config('constants.AUDIT_VOID'));
            })
            ->badge(function () {
                return Payroll::query()->where('pay_period_id', $this->record->pay_period_id)->where('status',config('constants.AUDIT_VOID'))
                ->count();
            })
                        ...
                        Actions::make([
                            Action::make('approve')
                                ...
                                ->action(function (Payroll $record, array $data) {
                                    self::handleAudit($record, config('constants.AUDIT_APPROVED'));
                                }),
                            Action::make('denied')
                                ->icon('heroicon-m-x-mark')
                                ->color('danger')
                                ->action(function (Payroll $record, array $data) {
                                    self::handleAudit($record, config('constants.AUDIT_DENIED'));
                                }),
                            Action::make('void')
                                ->icon('heroicon-m-x-mark')
                                ->color(Color::Gray)
                                ->action(function (Payroll $record, array $data) {
                                    self::handleAudit($record, config('constants.AUDIT_VOID'));
                                }),
                        ]),
                    ])->columns(1)
                ])  
    ...
    public function handleAudit(Payroll $record, $status)
    {
        ...
        // If successful, update or create audit record
        if ($result) {
        ...
        $record->save();
        $notificationColor = $status === config('constants.AUDIT_DENIED') || $status === config('constants.AUDIT_VOID') 
            ? 'danger' 
            : 'success';

        Notification::make()
            ->title('Payroll Audited')
    ...
    public function table(Table $table): Table
    {
        ...
        ->query(Payroll::query()->where('pay_period_id', $pay_period_id))
            ->columns([
                ...
                TextColumn::make('audit.remarks')->label('Auditor Remarks'),
                CheckboxColumn::make('is_audited')->label('Audited'),
                TextColumn::make('audit.audited_by')->label('Audited By'),
            ...
            ->actions([
                ...
                Tables\Actions\ViewAction::make()->infolist(fn(Infolist $infolist) => ViewEmployeePayrollToAuditTable::infolist($infolist))->modalWidth('8xl'),
            ])
            ->bulkActions([
                Tables\Actions\BulkActionGroup::make([
                    ...
                    ->action(function (Collection $records){
                        foreach ($records as $record) {
                            $record->status = config('constants.AUDIT_APPROVED');
                            $record->is_audited = true;
                            $result = $record->save();
                            ...
                            if ($result) {
                                $record->audit()->updateOrCreate(
                                    ...
                                    [
                                        'audited_by' => auth()->id(),
                                        'status' => config('constants.AUDIT_APPROVED'),
                                    ]
                                );
                            }
                        }
                        ...
                        ->color('success')
                        ->title('Payroll Audit')
                        ->body('Payroll Audited Successfully.')
                        ->send();
                    }),
                ]),
            ]);
    ...
    {
        return view('livewire.view-employee-payroll-to-audit-table');
    }
}
